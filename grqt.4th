...
: ?min_distance ( I dist -- )
DUP min_distance < IF 2DROP ELSE TO min_distance TO dot_n THEN ;
: calc_way { | dist adr }
...
: calc_dist { | dist adr x0 y0 }
		...
		dots2 @ dot_m - 0 DO 
			x0 y0 I xy 2@ 
			calc_distance DUP . DUP I SWAP ?min_distance dist + TO dist I dist ?min_distance
		LOOP dist CR ." distance: " . dist
	." minimal distance:" min_distance . ." to dot:" dot_n . ;
...
calc_dist S>D (D.) DROP way_txt -text!
